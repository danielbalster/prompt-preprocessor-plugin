...
// ── Test 5: !define with backtick shell command ───────────────────
console.log("Test 5: !define with backtick command")
{
  const result = await run("!define HOST `uname -n`\nhost: ${HOST}")
  const expected = "host: " + (process.env["HOST"] ?? "")
  assertEq(result, expected, "define backtick command")
}

// ── Test 6: !define with backtick — trailing newline stripped ──────
console.log("Test 6: !define backtick trailing newline stripped")
{
  const result = await run("!define VAL `printf 'hello'`\nextra")
  assertEq(result, "extra", "backtick trailing newline stripped")
  const val = process.env["VAL"]
  if (val !== "hello") { failed++; console.error(`FAIL [backtick trailing newline stripped] expected process.env.VAL="hello", got ${JSON.stringify(val)}`) }
  else passed++
}

// ── Test 7: !define with backtick — multi-word output ──────────────
console.log("Test 7: !define backtick multi-word output")
{
  const result = await run("!define INFO `uname -s -m`\nsys: ${INFO}")
  const expected = "sys: " + (process.env["INFO"] ?? "")
  assertEq(result, expected, "define backtick multi-word")
}

// ── Test 8: !define with backtick — empty output ───────────────────
console.log("Test 8: !define backtick empty output")
{
  const result = await run("!define BLANK `true`\nval: [${BLANK}]")
  assertEq(result, "val: []", "define backtick empty output")
}

// ── Test 9: ${VAR:default} when var is undefined ───────────────────
console.log("Test 9: ${VAR:default} undefined")
{
...
// ── Test 10: ${VAR:default} when var is defined ─────────────────────
console.log("Test 10: ${VAR:default} defined")
{
  const result = await run("Hello ${TESTVAR:fallback}", { TESTVAR: "real" })
  assertEq(result, "Hello real", "defined overrides default")
}

// ── Test 11: ${VAR} without default (undefined -> unchanged) ────────
console.log("Test 11: ${VAR} without default undefined")
{
...
// ── Test 12: defined(VAR) in expression ─────────────────────────────
console.log("Test 12: defined(VAR) true")
{
  const result = await run("!if defined(TEST8)\nVAR present\n!else\nVAR absent\n!endif", { TEST8: "1" })
  assertEq(result, "VAR present", "defined true")
}

// ── Test 13: defined(VAR) false ─────────────────────────────────────
console.log("Test 13: defined(VAR) false")
{
...
// ── Test 14: !defined(VAR) ─────────────────────────────────────────
console.log("Test 14: !defined(VAR)")
{
...
// ── Test 15: defined(VAR) && defined(OTHER) ───────────────────────
console.log("Test 15: defined() && defined()")
{
  const result = await run("!if defined(TEST11A) && defined(TEST11B)\nboth\n!else\nnot both\n!endif", { TEST11A: "1", TEST11B: "1" })
  assertEq(result, "both", "both defined")
}

// ── Test 16: defined(VAR) || defined(OTHER) ────────────────────────
console.log("Test 16: defined() || defined()")
{
...
// ── Test 17: exists("path") true ────────────────────────────────────
console.log("Test 17: exists() true")
{
  const result = await run('!if exists("prompt-preprocessor.ts")\nfile exists\n!else\nfile missing\n!endif')
  assertEq(result, "file exists", "exists true")
}

// ── Test 18: exists("path") false ───────────────────────────────────
console.log("Test 18: exists() false")
{
...
// ── Test 23: !error directive ────────────────────────────────────────
console.log("Test 23: !error directive")
{
  await assertThrows(
...
// ── Test 24: !shell silent (no output) ───────────────────────────────
console.log("Test 24: !shell silent")
{
  const result = await run("before\n!shell echo hello\nmiddle\n!shell echo world\nafter")
  assertEq(result, "before\nmiddle\nafter", "shell silent")
}

// ── Test 25: !shell>1 stdout only ────────────────────────────────────
console.log("Test 25: !shell>1 stdout")
{
  const result = await run("!shell>1 echo stdout only")
  assertEq(result, "stdout only", "shell stdout")
}

// ── Test 26: !shell>2 stderr only ────────────────────────────────────
console.log("Test 26: !shell>2 stderr")
{
  const result = await run("!shell>2 sh -c 'echo stderr only >&2'")
  assertEq(result, "stderr only", "shell stderr")
}

// ── Test 27: !shell> stdout and stderr ───────────────────────────────
console.log("Test 27: !shell> both")
{
  const result = await run("!shell> sh -c 'echo stdout; echo stderr >&2'")
  assertEq(result, "stdout\nstderr", "shell both")
}

// ── Test 28: !shell>1 with no output ─────────────────────────────────
console.log("Test 28: !shell>1 no output")
{
  const result = await run("!shell>1 true")
  assertEq(result, "", "shell no output")
}

// ── Test 29: Combined !define + !ifdef ───────────────────────────────
console.log("Test 29: !define + !ifdef")
{
  const result = await run("!define MODE debug\n!ifdef MODE\nenabled\n!else\ndisabled\n!endif")
  assertEq(result, "enabled", "define then ifdef")
}

// ── Test 30: !if expression with string comparison ───────────────────
console.log("Test 30: !if VAR == string")
{
  const result = await run('!if MODE == "debug"\nDEBUG\n!else\nRELEASE\n!endif', { MODE: "debug" })
  assertEq(result, "DEBUG", "var equals string")
}

// ── Test 31: !if expression with numeric comparison ──────────────────
console.log("Test 31: !if VAR >= number")
{
  const result = await run("!if VER >= 3\nok\n!else\nnok\n!endif", { VER: "5" })
  assertEq(result, "ok", "var >= number")
}

// ── Test 32: !if VAR truthy shorthand ────────────────────────────────
console.log("Test 32: !if VAR truthy")
{
  const result = await run("!if DEBUG\nenabled\n!else\ndisabled\n!endif", { DEBUG: "1" })
  assertEq(result, "enabled", "truthy var")
}

// ── Test 33: !if !VAR shorthand (empty -> false) ─────────────────────
console.log("Test 33: !if !VAR falsey")
{
...
// ── Test 34: !if VAR contains substring ~ ────────────────────────────
console.log("Test 34: !if VAR ~ substring")
{
  const result = await run('!if TEXT ~ "hello"\nmatch\n!else\nno match\n!endif', { TEXT: "hello world" })
  assertEq(result, "match", "substring match")
}

// ── Test 35: defined() with !elif ────────────────────────────────────
console.log("Test 35: defined() in !elif")
{
...
// ── Test 36: exists() with !elif ─────────────────────────────────────
console.log("Test 36: exists() in !elif")
{
  const result = await run('!if exists("no-such-file.txt")\nfirst\n!elif exists("prompt-preprocessor.ts")\nsecond\n!else\nthird\n!endif')
  assertEq(result, "second", "exists in elif")
}

// ── Test 37: Multiple !define directives ─────────────────────────────
console.log("Test 37: multiple !define")
{
  const result = await run("!define A 1\n!define B 2\n${A} + ${B}")
  assertEq(result, "1 + 2", "multiple defines")
}

// ── Test 38: Compound expression with functions ──────────────────────
console.log("Test 38: compound expression")
{
  const result = await run("!if defined(A) || defined(B)\nfound\n!else\nnot found\n!endif", { A: "1" })
  assertEq(result, "found", "compound or")
}

// ── Test 39: !if VAR != string ───────────────────────────────────────
console.log("Test 39: !if VAR != string")
{
  const result = await run('!if MODE != "debug"\nRELEASE\n!else\nDEBUG\n!endif', { MODE: "release" })
  assertEq(result, "RELEASE", "var not equal")
}

// ── Test 40: !error inside false branch must NOT fire ─────────────────
console.log("Test 40: !error inside false branch silent")
{
  const result = await run('!if !exists("prompt-preprocessor.ts")\n!error should not fire\n!else\nall good\n!endif')
  assertEq(result, "all good", "error in false branch")
}

// ── Test 41: !error inside true branch DOES fire ──────────────────────
console.log("Test 41: !error inside true branch fires")
{
  await assertThrows(
